...
export class BookmarkController {
    ...
    @Post("create")
    async createBookmark(@GetUser('id') id:number, @Body() bookmark:CreateBookmarkDto){
    ...
    @Delete("delete/:id")
    async deleteBookmark (@GetUser('id') id:number,@Param("id") bookmarkId:number){
        return this.bookmarkService.deleteBookmark(id,Number(bookmarkId))
    }

    @Patch("update/:id")
    async updateBookmark (@GetUser('id') id:number,@Param("id") bookmarkId:number,@Body() data:UpdateBookmarkDto){
